...
| **Vector Search & RAG** | In-memory HNSW/IVF and disk-backed **DiskIVF** (100M+ vectors, 194× RAM reduction via scalar quantization, SOAR boundary recall), SurrealQL and SPARQL vector operators, hybrid text+vector queries, stored-value filtering, Vector K-NN in LPG, and a complete RAG pipeline with Knowledge Graph provenance | [Vector Search](vector-search.md) |
...
- If you are building a **RAG pipeline or AI application**: start with [Vector Search](vector-search.md). For corpora above ~500K vectors, see the [DiskIVF](vector-search.md#diskivf-disk-backed-vector-index) section.
